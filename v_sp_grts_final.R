...
for(i in 1:n_repeats){
  ...
  # Calcul des quantiles pour définir les seuils
  threshold <- quantile(initial_prediction, probs = 0.75)
  # Classification binaire en utilisant les seuils
  ...
  for(j in 1:iterations){
    ...
    distrib_points <- rasterToPoints(binary_initial_prediction_cropped)
    ...
    coord = data.frame(latitude = eqprob_strat$sites_base$lat_WGS84, longitude = eqprob_strat$sites_base$lon_WGS84)
    coord_sp = SpatialPointsDataFrame(coords = coord[,c("longitude","latitude")],data =coord, proj4string =  CRS("+proj=longlat +datum=WGS84"))
    plot(binary_virtual_species_cropped, col = c("darkgrey","darkblue"))
    plot(coord_sp, pch = 20, cex = 2, add = T, col = "darkorange")
    ###
...
boxplot(transposed_dataframe, main = "GRTS - 25 échantillons",
...
boxplot(transposed_pres, main = "GRTS - 25 échantillons",
        xlab = "Nombre d'itérations", ylab = "Nombres de nouvelles présences trouvées")


#save.image("C:/Users/charl/Desktop/FINAL_GRTS.RData")
test = SpatialPoints(sampling_sites)
plot(binary_virtual_species_cropped, col = c("darkgrey","darkblue"))
plot(coord_sp, pch = 20, cex = 2, add = T, col = "darkorange")
plot(binary_initial_prediction,col = c("darkgrey","darkblue"))
plot(binary_iterative_prediction,col = c("darkgrey","darkblue"))
plot(iterative_prediction, col = hcl.colors(80,"viridis"))


plot(full_predictions_list[[1]][[2]], col = c("darkgrey","darkblue"))
plot(full_predictions_list[[3]][[2]], col = c("darkgrey","darkblue"))
plot(full_predictions_list[[5]][[2]], col = c("darkgrey","darkblue"))
